...
model.eval()
top_N = 5
speakerPredictor = Flask(__name__)
...
@speakerPredictor.route('/test', methods=['POST', 'GET'])
def predict():
    res = dict()
    talk = str(request.form.get("talk"))
    if len(talk) != 0:
        ...
        for i in range(top_N):
            top_n_prob.append((config.class_list[ranks[i]], str(prob[ranks[i]])))
        res['status'] = '200 OK'
        res['data'] = top_n_prob
    else:
        res['status'] = '500'
...
if __name__ == '__main__':
    ...
    speakerPredictor.run(debug=True, threaded=True, host='0.0.0.0', port=80)
